...
# ============== CORS MIDDLEWARE - CONFIGURAÇÃO CORRIGIDA ==============
# 
# MUDANÇA IMPORTANTE: Em vez de usar .split(','), vamos processar corretamente
# para remover espaços em branco e garantir que funcione
#
cors_origins_str = os.environ.get('CORS_ORIGINS', '*')

# Se for asterisco, aceita todas as origens
if cors_origins_str == '*':
    cors_origins = ['*']
else:
    # Processa a string: divide por vírgula e remove espaços em branco
    cors_origins = [origin.strip() for origin in cors_origins_str.split(',') if origin.strip()]

print(f"🔧 CORS Origins configuradas: {cors_origins}")  # Log para debug

app.add_middleware(
    CORSMiddleware,
    allow_origins=cors_origins,  # Lista já processada
    allow_credentials=True,
    allow_methods=["GET", "POST", "PUT", "DELETE", "PATCH", "OPTIONS"],  # Especificando métodos
    allow_headers=["*"],
    expose_headers=["*"],
...
def generate_payment_link(payment_id: str, amount: float, client_name: str) -> str:
    base_url = "https://fotivaappp.onrender.com"

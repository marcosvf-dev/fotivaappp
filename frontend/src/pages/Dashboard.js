  ...
  const metricCards = [
    {
      ...
      value: `R$ ${metrics?.monthly_revenue?.toLocaleString('pt-BR', { minimumFractionDigits: 2 }) || '0,00'}`,
      icon: DollarSign,
      trend: `+${metrics?.revenue_trend || 0}% no mês anterior`,
      color: 'bg-[#E8F5E9]',
      iconColor: 'text-[#4A9B6E]'
    },
    {
      title: 'Eventos Confirmados',
      value: metrics?.confirmed_events || 0,
      icon: Calendar,
    ...
    },
    {
      ...
      value: metrics?.photos_delivered || 0,
      icon: Image,
      trend: 'Este mês',
      color: 'bg-purple-50',
    ...
    },
    {
      ...
      value: `R$ ${metrics?.pending_payments?.toLocaleString('pt-BR', { minimumFractionDigits: 2 }) || '0,00'}`,
      icon: DollarSign,
      trend: '5 clientes',
      color: 'bg-orange-50',
      ...
      <div className="space-y-8">
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <LineChart data={metrics?.monthly_revenue_chart || []}>
              ...
              {metrics.upcoming_events.map((event, index) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="text-sm font-medium text-[#111827] truncate">{event.name}</h3>
                    <p className="text-xs text-[#6B7280] mt-1">{event.client_name}</p>
                  </div>
                  <div className="text-right flex-shrink-0">
                    <p className="text-sm font-medium text-[#111827]">{event.date}</p>
